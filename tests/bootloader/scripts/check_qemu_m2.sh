...
LOG_FILE="build/qemu-m2-debug.log"
rm -f "$LOG_FILE"
...
if [ -f "$LOG_FILE" ] && grep -q 'KERNEL_OK' "$LOG_FILE"; then
    echo "PASS: QEMU kernel handoff verified (KERNEL_OK)"
    exit 0
fi

echo "FAIL: KERNEL_OK marker not detected"
if [ -f "$LOG_FILE" ]; then
    cat "$LOG_FILE"
fi
exit 1
