...
LOG_FILE="build/qemu-hi-t1-debug.log"
rm -f "$LOG_FILE"
...
if grep -q 'HI_INIT_OK' "$LOG_FILE"; then
    echo "PASS: hardware interrupt initialization verified (HI_INIT_OK)"
    exit 0
fi

echo "FAIL: hardware interrupt init marker HI_INIT_OK not detected"
cat "$LOG_FILE"
exit 1
