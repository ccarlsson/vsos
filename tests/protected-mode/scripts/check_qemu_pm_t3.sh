...
DISK_IMG="${1:-build/disk-pm-t3.img}"
...
LOG_FILE="build/qemu-pm-t3-debug.log"
rm -f "$LOG_FILE"
...
if grep -q 'P2' "$LOG_FILE"; then
    echo "PASS: protected-mode transition setup failure path verified (P2)"
    exit 0
fi

echo "FAIL: protected-mode marker P2 not detected"
cat "$LOG_FILE"
exit 1